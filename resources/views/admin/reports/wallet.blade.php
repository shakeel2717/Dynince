                    ...
                    <table class="table table-hover">
                        ...
                        @else
                            ...
                            <td data-label="{{ __('locale.Status') }}">
                                @if ($trx->status == 1)
                                    <span
                                        class="badge rounded-pill badge-light-success me-1">{{ __('locale.Completed') }}</span>
                                @elseif($trx->status == 2)
                                    <span
                                        class="badge rounded-pill badge-light-warning me-1">{{ __('locale.Pending') }}</span>
                                @elseif($trx->status == 3)
                                    <span
                                        class="badge rounded-pill badge-light-danger me-1">{{ __('locale.Rejected') }}</span>
                                @endif
                            </td>
                ...
                <div class="modal-content">
                    ...
                    <form class="deposit-form" action="{{ route('admin.report.wallet.transfer.funding.approve') }}"
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-sm btn-success">{{ __('locale.Confirm') }}</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('admin.report.wallet.cancel') }}" method="POST">
                        ...
                        <input type="hidden" name="trx">
                        <div class="modal-body">
                            <p>Are you sure you want to cancel transfer?</p>
                        </div>
                        {{-- <div class="modal-body">
                        <strong>@lang('Reason of Rejection')</strong>
                        <textarea name="details" class="form-control pt-3" rows="3" placeholder="@lang('Provide the Details')" required=""></textarea>
        ...
        <form action="{{ route('admin.report.' . $page . '.search') }}" method="GET" class=" float-sm-right bg-white">
            <div class="input-group">
                <input type="text" name="search" class="form-control"
                    placeholder="{{ __('locale.TRX / Username') }}" value="{{ $search ?? '' }}">
