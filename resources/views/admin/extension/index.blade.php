                            ...
                            @foreach ($extensions as $extension)
                                <tr>
                                    ...
                                    <td data-label="{{ __('locale.Status') }}">
                                        @if ($extension->dev == 1)
                                            <span class="badge bg-secondary">In Planning Stage</span>
                                        @elseif ($extension->dev == 2)
                                            <span class="badge bg-info">In Development Stage</span>
                                        @else
                                            ...
                                            @else
                                                <span
                                                    class="badge bg-light-success">{{ __('locale.Not Installed') }}</span>
                                            @endif
                                        @endif
                                    </td>
                                    <td data-label="{{ __('locale.Action') }}">
                                        @if ($extension->installed == 1)
                                            ...
                                            <a href="{{ route('admin.extensions.activater', [$extension->product_id]) }}"
                                                ...
                                                data-bs-placement="top" title="{{ __('locale.Re-Verify License') }}">
                                        ...
                                        @else
                                            @if ($extension->activated == 0)
                                                <a href="{{ route('admin.extensions.activater', [$extension->product_id]) }}"
                                                    class="btn btn-icon btn-success ms-1" data-bs-toggle="tooltip"
                                                    data-bs-placement="top" title="{{ __('locale.Verify License') }}">
                                                    <i class="bi bi-check-lg"></i>
                                                </a>
                                            @else
            ...
            <div class="modal-content">
                ...
                <form method="POST">
                    ...
                    <div class="modal-body">
                        <div class="col">
                            <label class="col-md-12 control-label fw-bold">{{ __('locale.Script') }} <span
                                    class="text-danger">*</span></label>
                            <div class="col-md-12">
                                <textarea name="script" class="form-control" rows="8"
                                    placeholder="{{ __('locale.Paste your script with proper key') }}"></textarea>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.extensions.activate') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>{{ __('locale.Are you sure to activate') }} <span class="fw-bold extension-name"></span>
                            {{ __('locale.extension') }}?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-dark"
                            data-bs-dismiss="modal">{{ __('locale.Close') }}</button>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.extensions.deactivate') }}" method="POST">
                    ...
                    <input type="hidden" name="id">
                    <div class="modal-body">
                        <p>{{ __('locale.Are you sure to disable') }} <span class="fw-bold extension-name"></span>
                            {{ __('locale.extension') }}?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-dark"
