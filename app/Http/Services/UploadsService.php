...
use App\Events\BlogPostEdited;
use App\Events\BlogPostWillBeDeleted;
use App\Events\UploadedImage;
use WebDevEtc\BlogEtc\Helpers;
use WebDevEtc\BlogEtc\Interfaces\LegacyGetImageFileInterface;
use App\Models\Blog\Post;
use App\Models\Blog\UploadedPhoto;
use App\Http\Repositories\UploadedPhotosRepository;
...
class UploadsService
{
    ...
    public function legacyProcessUploadedImagesSingle(UploadImageRequest $request)
    {
        ...
        if (isset($sizes_to_upload['blogetc_full_size']) && 'true' === $sizes_to_upload['blogetc_full_size']) {
            $uploaded_image_details['blogetc_full_size'] = $this->legacyUploadAndResize(
                null,
        ...
        foreach ((array) config('blogetc.image_sizes') as $size => $image_size_details) {
            ...
            $uploaded_image_details[$size] = $this->legacyUploadAndResize(
                null,
                $request->get('image_title'),
                $image_size_details,
                $photo
    ...
    public function legacyProcessUploadedImages(LegacyGetImageFileInterface $request, Post $new_blog_post)
    {
        ...
        foreach ((array) config('blogetc.image_sizes') as $size => $image_size_details) {
            if ($image_size_details['enabled'] && $photo = $request->get_image_file($size)) {
                $uploaded_image = $this->legacyUploadAndResize(
                    $new_blog_post,
                    $new_blog_post->title,
                    $image_size_details,
                    $photo
