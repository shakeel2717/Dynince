...
class HomeController extends Controller
{
    ...
    {
        $template = FrontendTemplates::where('status', 1)->first();
        $page = FrontendPages::where('template_id', $template->template_id)->where('title', $title)->first();
        $sections = FrontendSections::where('template_id', $template->template_id)->where('page_id', '1')->get();
        $images = frontend_images::where('template_id', $template->template_id)->where('page_id', '1')->get();
        $page_title = $page->name;
        $platform['frontend'] = json_decode(Platform::where('option', 'frontend')->first()->settings);
        $platform = arrayToObject($platform);
        return view('frontends.' . $template->title . '.' . $page->title, compact('page_title', 'sections', 'images', 'template', 'platform'));
    }
