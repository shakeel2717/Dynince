...
class CronController extends Controller
{
            ...
            foreach ($transactions as $transaction) {
                ...
                if ($req != null && $req['status'] == 'ok') {
                    if ($this->provider == 'kucoin') {
                        ...
                        $trx->user->notify(new DepositNote($trx, 'trading-wallet-deposit-successful'));
                    } else {
                        ...
                        $adminNotification->click_url = route('admin.report.wallet.deposit');
                        $adminNotification->save();
                        $transaction->user->notify(new DepositNote($transaction, 'trading-wallet-deposit-successful'));
                    }
                }
            }
        }
    ...
    public function staking_profit()
    {
        ...
        foreach ($stakeCoins as $coin) {
            if (StakingLog::where('coin_id', $coin->id)->where('status', 1)->exists()) {
                $stakeLog = StakingLog::where('coin_id', $coin->id)->where('status', 1)->first();
                if ($date > $stakeLog->end_date) {
                    $stakeLog->status = 2;
                } else {
                    if ($stakeLog->last_stake_date != null) {
                        if ($stakeLog->status == 1 && $date >= $stakeLog->last_stake_date && $stakeLog->end_date >= $stakeLog->last_stake_date) {
                            $stakeLog->last_profit = $stakeLog->cost * ($coin->daily_profit / 100);
                            $stakeLog->total_profit += $stakeLog->last_profit;
                            $stakeLog->last_stake_date = $stakeLog->updated_at->addDays(1);
                        }
                    } else {
                        if ($stakeLog->status == 1 && $date < $stakeLog->end_date) {
                            $stakeLog->last_profit = $stakeLog->cost * ($coin->daily_profit / 100);
                            $stakeLog->total_profit += $stakeLog->last_profit;
                            $stakeLog->last_stake_date = $date->addDays(1);
                        }
                    }
                }
                $stakeLog->save();
