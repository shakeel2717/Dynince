    ...
    public function logoIconUpdate(Request $request)
    {
        ...
        if ($request->hasFile('favicon_dark')) {
            try {
                ...
                $size = explode('x', imagePath()['favicon']['size']);
                Image::make($request->favicon_dark)->resize($size[0], $size[1])->save($path . '/favicon-dark.png');
            } catch (\Exception $exp) {
                $notify[] = ['error', 'Dark Favicon could not be uploaded.'];
                return back()->withNotify($notify);
            }
        }
